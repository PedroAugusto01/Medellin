(function ($) {
    ...
    var spinner = function () {
        setTimeout(function () {
            if ($('#spinner').length > 0) {
                $('#spinner').removeClass('show');  // Remove the 'show' class to hide the spinner
            }
        }, 1);  // It will disappear almost immediately after the page loads
    };
    spinner();
    ...
    $(".testimonial-carousel").owlCarousel({
        ...
        smartSpeed: 1000,
        margin: 24,
        dots: true,  // corrected from 'sniperse' to 'true'
        loop: true,
        nav: true,
        navText: [
...
function calculateFarmToMuni() {
    ...
    for(var item in result) {
        ...
        let itemValue = parseInt(((parseInt($(itemHtml).val() , 10) * (1 - (result[item]['porcentagemBancadaZero']/100))) / (result[item]['valorBancada'] / 10)), 10);
        if (!isNaN(itemValue)) {
...
function calculateMuniToFarm() {
    ...
    for(var item in result) {
        let itemHtml = "#"+result[item]['qtyCalcBancada']
        let priceHtml = "#"+result[item]['priceCalcBancada']
        let itemValue = parseInt(((parseInt($(priceHtml).val() , 10) * (result[item]['valorBancada'] / 10)) / (1 - (result[item]['porcentagemBancadaZero']/100))), 10) 
